  ...
  describe("deterministic serialization", () => {
    it("serializes stable JSON snapshots from fixed doubles", () => {
      const createWithFixedDoubles = (): QuatrainCandidate => {
        const result = createQuatrainCandidateWithCollaborators(
          candidateFactoryInput(),
          {
        ...
        if (!result.ok) {
          throw new Error(
            "Expected deterministic candidate creation to succeed",
          );
        }
      ...
      const firstSnapshot = toQuatrainCandidateSnapshot(
        createWithFixedDoubles(),
      );
      const secondSnapshot = toQuatrainCandidateSnapshot(
        createWithFixedDoubles(),
      );

      assert.deepEqual(firstSnapshot, secondSnapshot);
      assert.deepEqual(
        JSON.parse(JSON.stringify(firstSnapshot)),
        firstSnapshot,
      );
